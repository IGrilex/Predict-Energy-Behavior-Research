...
class Attention(nn.Module):
    ...
    def forward(self, lstm_output):
        ...
        attn_weights = torch.softmax(attn_weights, dim=1) # softmax over sequence
        context = torch.sum(lstm_output * attn_weights, dim=1)  # weighted sum over time dimension => [batch_size, hidden_dim]
        return context

# RNN with Attention and Increased Capacity
class RNNWithAttention(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, num_layers=2, fc_hidden_dim=64, output_dim=1):
        """
        input_dim   : number of features per timestep.
        hidden_dim  : number of hidden units in LSTM (increased capacity).
        num_layers  : number of stacked LSTM layers.
        fc_hidden_dim: number of neurons in FC layer after attention.
...
def create_rnn_model(input_dim, hidden_dim=128, num_layers=2, fc_hidden_dim=64, output_dim=1):
    """
    Create and return an instance of the RNN model (LSTM with Attention).
    """
    return RNNWithAttention(input_dim, hidden_dim, num_layers, fc_hidden_dim, output_dim)
